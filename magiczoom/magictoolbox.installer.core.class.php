<?php
    ...
    class MagicToolboxCoreInstallerClass {
            ...
            if($dir = @opendir($src)) {
                while (($file = readdir($dir))!==false) {
                    ...
                    if(is_dir($src . '/' . $file)) {
                        ...
                        } else if(($file == 'magiczoom.css' || $file == 'magicscroll.css') && $this->resDir != '') {
                            $fileExists = file_exists($dest.'/'.$file);
                            if($fileExists && !$overwrite) continue;
                            ...
                            case 'DIR':
                                if(file_exists($this->dir.'/'.$matches[3]) && !in_array($matches[3], $exclude)) {
                                    if(!$this->isEmptyDir($this->dir.'/'.$matches[3])) {
                                        //if(preg_match('/^magic(?:toolbox|zoom(?:plus)?|thumb|magnify(?:plus)?|touch|360(?:plus)?|scroll|slideshow)$/i', basename($matches[3]))) {
        ...
        function sendStat($mode = '') {
            ...
            if('working' == 'v4.9.25' || 'working' == 'v4.5.30') {
                return;
            }
            ...
            $path = "api/stat/?action={$mode}&tool_name=magiczoom&license=trial&tool_version=v4.5.30&module_version=v4.9.25&platform_name=magento&platform_version={$platformVersion}&url={$url}";
